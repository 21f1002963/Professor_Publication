...
const Professor = require('../Professor'); // Assuming you have the Professor model
...
router.post('/faculty/:nodeId', async (req, res) => {
  try {
    ...
    const existingProfessor = await Professor.findOne({
      $or: [
        { node_id: nodeId },
        { source_url: `https://backup.pondiuni.edu.in/PU_Establishment/profile_view/?node=${nodeId}` }
      ]
    });

    if (existingProfessor && !overwrite) {
      return res.status(409).json({
        success: false,
        message: 'Faculty data already exists. Use overwrite=true to update.',
        existingData: {
          name: existingProfessor.name,
          email: existingProfessor.email,
          department: existingProfessor.department
        }
      });
    }
    ...
    // Transform scraped data to match Professor schema
    const professorData = {
      ...
      research_interests: scrapedData.research_interests,
    ...
    res.status(201).json({
      success: true,
      message: `Faculty data ${overwrite ? 'updated' : 'imported'} successfully`,
      data: {
        ...
        node_id: nodeId,
        scraped_fields: Object.keys(scrapedData).filter(key => scrapedData[key] && scrapedData[key].length > 0)
      }
    });
...
router.post('/faculty/batch', async (req, res) => {
  try {
    ...
    const results = await scraper.scrapeMultipleFaculty(nodeIds);
    const importResults = {
      imported: [],
      updated: [],
      errors: [],
    ...
    // Process each successfully scraped faculty
    for (const scrapedData of results.success) {
      try {
        const existingProfessor = await Professor.findOne({
          $or: [
            { node_id: scrapedData.node_id },
        ...
        if (existingProfessor && overwrite) {
          await Professor.findByIdAndUpdate(existingProfessor._id, { $set: professorData });
          importResults.updated.push({
            professor_id: existingProfessor._id,
            name: scrapedData.name,
            node_id: scrapedData.node_id
          });
        } else {
    ...
    // Add scraping errors to results
